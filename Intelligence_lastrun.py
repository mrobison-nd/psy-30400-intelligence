...
expName = 'Intelligence'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='G:\\My Drive\\Teaching\\FA 26 PSY 30400\\Experiments\\IQ\\Intelligence_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "gc_intro" ---
    text_10 = visual.TextStim(win=win, name='text_10',
        text='For the next set of items, there is no time limit. \n\nFor the first 10 items, you will be given a general knowledge question. \n\nFor the next 10 items, you will be given a word and asked to identify a SYNONYM for it. \n\nFor the last 10 items, you will be given a word and asked to identify an ANTONYM for it.\n\nPress the spacebar to continue.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.03, wrapWidth=None, ori=0.0, 
        color='black', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Initialize components for Routine "gk_trial" ---
    text_7 = visual.TextStim(win=win, name='text_7',
        ...
        languageStyle='LTR',
        depth=0.0);
    ans1_text_2 = visual.TextStim(win=win, name='ans1_text_2',
        text='',
        font='Arial',
        ...
        languageStyle='LTR',
        depth=-1.0);
    ans2_text_2 = visual.TextStim(win=win, name='ans2_text_2',
        text='',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.03, wrapWidth=None, ori=0.0, 
        color='black', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-2.0);
    ans3_text_2 = visual.TextStim(win=win, name='ans3_text_2',
        text='',
        font='Arial',
        pos=(0, -.1), draggable=False, height=0.03, wrapWidth=None, ori=0.0, 
        color='black', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-3.0);
    ans4_text_2 = visual.TextStim(win=win, name='ans4_text_2',
        text='',
        font='Arial',
    ...
    # --- Initialize components for Routine "syn_ant_trial" ---
    ...
    # --- Prepare to start Routine "gc_intro" ---
    # create an object to store info about Routine gc_intro
    gc_intro = data.Routine(
        name='gc_intro',
        components=[text_10, key_resp],
    )
    gc_intro.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_resp.rt = []
    _key_resp_allKeys = []
    # store start times for gc_intro
    gc_intro.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    gc_intro.tStart = globalClock.getTime(format='float')
    gc_intro.status = STARTED
    thisExp.addData('gc_intro.started', gc_intro.tStart)
    gc_intro.maxDuration = None
    # keep track of which components have finished
    gc_introComponents = gc_intro.components
    for thisComponent in gc_intro.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "gc_intro" ---
    gc_intro.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            gc_intro.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in gc_intro.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "gc_intro" ---
    for thisComponent in gc_intro.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for gc_intro
    gc_intro.tStop = globalClock.getTime(format='float')
    gc_intro.tStopRefresh = tThisFlipGlobal
    thisExp.addData('gc_intro.stopped', gc_intro.tStop)
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "gc_intro" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    gk_loop = data.TrialHandler2(
        name='gk_loop',
        nReps=1.0, 
        method='sequential', 
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('gk_questions.xlsx'), 
        seed=None, 
    )
    thisExp.addLoop(gk_loop)  # add the loop to the experiment
    thisGk_loop = gk_loop.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisGk_loop.rgb)
    if thisGk_loop != None:
        for paramName in thisGk_loop:
            globals()[paramName] = thisGk_loop[paramName]
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    
    for thisGk_loop in gk_loop:
        currentLoop = gk_loop
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
        ...
        # --- Prepare to start Routine "gk_trial" ---
        # create an object to store info about Routine gk_trial
        gk_trial = data.Routine(
            name='gk_trial',
            components=[text_7, ans1_text_2, ans2_text_2, ans3_text_2, ans4_text_2, mouse_11],
        )
        gk_trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        text_7.setText(question
        )
        ans1_text_2.setText(opt1)
        ans2_text_2.setText(opt2)
        ans3_text_2.setText(opt3)
        ans4_text_2.setText(opt4)
        ...
        gotValidClick = False  # until a click is received
        # store start times for gk_trial
        gk_trial.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        gk_trial.tStart = globalClock.getTime(format='float')
        gk_trial.status = STARTED
        thisExp.addData('gk_trial.started', gk_trial.tStart)
        gk_trial.maxDuration = None
        # keep track of which components have finished
        gk_trialComponents = gk_trial.components
        for thisComponent in gk_trial.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "gk_trial" ---
        # if trial has changed, end Routine now
        if isinstance(gk_loop, data.TrialHandler2) and thisGk_loop.thisN != gk_loop.thisTrial.thisN:
            continueRoutine = False
        gk_trial.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            # if ans1_text_2 is starting this frame...
            if ans1_text_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                ans1_text_2.frameNStart = frameN  # exact frame index
                ans1_text_2.tStart = t  # local t and not account for scr refresh
                ans1_text_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(ans1_text_2, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'ans1_text_2.started')
                # update status
                ans1_text_2.status = STARTED
                ans1_text_2.setAutoDraw(True)
            
            # if ans1_text_2 is active this frame...
            if ans1_text_2.status == STARTED:
                # update params
                pass
            
            # *ans2_text_2* updates
            
            # if ans2_text_2 is starting this frame...
            if ans2_text_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                ans2_text_2.frameNStart = frameN  # exact frame index
                ans2_text_2.tStart = t  # local t and not account for scr refresh
                ans2_text_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(ans2_text_2, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'ans2_text_2.started')
                # update status
                ans2_text_2.status = STARTED
                ans2_text_2.setAutoDraw(True)
            
            # if ans2_text_2 is active this frame...
            if ans2_text_2.status == STARTED:
                # update params
                pass
            
            # *ans3_text_2* updates
            
            # if ans3_text_2 is starting this frame...
            if ans3_text_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                ans3_text_2.frameNStart = frameN  # exact frame index
                ans3_text_2.tStart = t  # local t and not account for scr refresh
                ans3_text_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(ans3_text_2, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'ans3_text_2.started')
                # update status
                ans3_text_2.status = STARTED
                ans3_text_2.setAutoDraw(True)
            
            # if ans3_text_2 is active this frame...
            if ans3_text_2.status == STARTED:
                # update params
                pass
            
            # *ans4_text_2* updates
            
            # if ans4_text_2 is starting this frame...
            if ans4_text_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                ans4_text_2.frameNStart = frameN  # exact frame index
                ans4_text_2.tStart = t  # local t and not account for scr refresh
                ans4_text_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(ans4_text_2, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'ans4_text_2.started')
                # update status
                ans4_text_2.status = STARTED
                ans4_text_2.setAutoDraw(True)
            
            # if ans4_text_2 is active this frame...
            if ans4_text_2.status == STARTED:
                # update params
                pass
            # *mouse_11* updates
            ...
            if mouse_11.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        # check if the mouse was inside our 'clickable' objects
                        gotValidClick = False
                        clickableList = environmenttools.getFromNames([ans1_text_2, ans2_text_2, ans3_text_2, ans4_text_2], namespace=locals())
                        for obj in clickableList:
                            # is this object clicked on?
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                gk_trial.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in gk_trial.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "gk_trial" ---
        for thisComponent in gk_trial.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for gk_trial
        gk_trial.tStop = globalClock.getTime(format='float')
        gk_trial.tStopRefresh = tThisFlipGlobal
        thisExp.addData('gk_trial.stopped', gk_trial.tStop)
        # store data for gk_loop (TrialHandler)
        gk_loop.addData('mouse_11.x', mouse_11.x)
        gk_loop.addData('mouse_11.y', mouse_11.y)
        gk_loop.addData('mouse_11.leftButton', mouse_11.leftButton)
        gk_loop.addData('mouse_11.midButton', mouse_11.midButton)
        gk_loop.addData('mouse_11.rightButton', mouse_11.rightButton)
        gk_loop.addData('mouse_11.time', mouse_11.time)
        gk_loop.addData('mouse_11.clicked_name', mouse_11.clicked_name)
        # the Routine "gk_trial" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed 1.0 repeats of 'gk_loop'
    ...
    # set up handler to look after randomisation of conditions etc
    syn_ant_loop = data.TrialHandler2(
        name='syn_ant_loop',
        nReps=1.0, 
        method='sequential', 
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('syn_ant_questions.xlsx'), 
        seed=None, 
    )
    thisExp.addLoop(syn_ant_loop)  # add the loop to the experiment
    thisSyn_ant_loop = syn_ant_loop.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisSyn_ant_loop.rgb)
    if thisSyn_ant_loop != None:
        for paramName in thisSyn_ant_loop:
            globals()[paramName] = thisSyn_ant_loop[paramName]
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    
    for thisSyn_ant_loop in syn_ant_loop:
        currentLoop = syn_ant_loop
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        ...
        # --- Prepare to start Routine "syn_ant_trial" ---
        # create an object to store info about Routine syn_ant_trial
        syn_ant_trial = data.Routine(
            name='syn_ant_trial',
            components=[text_8, text_9, ans1_text, ans2_text, ans3_text, ans4_text, ans5_text, mouse_12],
        )
        syn_ant_trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        text_8.setText(question)
        text_9.setText(word)
        ans1_text.setText(opt1)
        ans2_text.setText(opt2)
        ans3_text.setText(opt3)
        ans4_text.setText(opt4)
        ans5_text.setText(opt5)
        ...
        gotValidClick = False  # until a click is received
        # store start times for syn_ant_trial
        syn_ant_trial.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        syn_ant_trial.tStart = globalClock.getTime(format='float')
        syn_ant_trial.status = STARTED
        thisExp.addData('syn_ant_trial.started', syn_ant_trial.tStart)
        syn_ant_trial.maxDuration = None
        # keep track of which components have finished
        syn_ant_trialComponents = syn_ant_trial.components
        for thisComponent in syn_ant_trial.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "syn_ant_trial" ---
        # if trial has changed, end Routine now
        if isinstance(syn_ant_loop, data.TrialHandler2) and thisSyn_ant_loop.thisN != syn_ant_loop.thisTrial.thisN:
            continueRoutine = False
        syn_ant_trial.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            # if ans4_text is starting this frame...
            if ans4_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'ans4_text.started')
                # update status
                ans4_text.status = STARTED
                ans4_text.setAutoDraw(True)
            
            # if ans4_text is active this frame...
            if ans4_text.status == STARTED:
                # update params
                pass
            
            # *ans5_text* updates
            
            # if ans5_text is starting this frame...
            if ans5_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'ans5_text.started')
                # update status
                ans5_text.status = STARTED
                ans5_text.setAutoDraw(True)
            
            # if ans5_text is active this frame...
            if ans5_text.status == STARTED:
                # update params
                pass
            # *mouse_12* updates
            ...
            if mouse_12.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        # check if the mouse was inside our 'clickable' objects
                        gotValidClick = False
                        clickableList = environmenttools.getFromNames([ans1_text, ans2_text, ans3_text, ans4_text, ans5_text], namespace=locals())
                        for obj in clickableList:
                            # is this object clicked on?
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                syn_ant_trial.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in syn_ant_trial.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "syn_ant_trial" ---
        for thisComponent in syn_ant_trial.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for syn_ant_trial
        syn_ant_trial.tStop = globalClock.getTime(format='float')
        syn_ant_trial.tStopRefresh = tThisFlipGlobal
        thisExp.addData('syn_ant_trial.stopped', syn_ant_trial.tStop)
        # store data for syn_ant_loop (TrialHandler)
        syn_ant_loop.addData('mouse_12.x', mouse_12.x)
        syn_ant_loop.addData('mouse_12.y', mouse_12.y)
        syn_ant_loop.addData('mouse_12.leftButton', mouse_12.leftButton)
        syn_ant_loop.addData('mouse_12.midButton', mouse_12.midButton)
        syn_ant_loop.addData('mouse_12.rightButton', mouse_12.rightButton)
        syn_ant_loop.addData('mouse_12.time', mouse_12.time)
        syn_ant_loop.addData('mouse_12.clicked_name', mouse_12.clicked_name)
        # the Routine "syn_ant_trial" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed 1.0 repeats of 'syn_ant_loop'
