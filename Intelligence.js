...
flowScheduler.add(gc_introRoutineBegin());
flowScheduler.add(gc_introRoutineEachFrame());
flowScheduler.add(gc_introRoutineEnd());
const gk_loopLoopScheduler = new Scheduler(psychoJS);
flowScheduler.add(gk_loopLoopBegin(gk_loopLoopScheduler));
flowScheduler.add(gk_loopLoopScheduler);
flowScheduler.add(gk_loopLoopEnd);


const syn_ant_loopLoopScheduler = new Scheduler(psychoJS);
flowScheduler.add(syn_ant_loopLoopBegin(syn_ant_loopLoopScheduler));
flowScheduler.add(syn_ant_loopLoopScheduler);
flowScheduler.add(syn_ant_loopLoopEnd);
...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'gk_questions.xlsx', 'path': 'gk_questions.xlsx'},
    {'name': 'syn_ant_questions.xlsx', 'path': 'syn_ant_questions.xlsx'},
...
async function experimentInit() {
  ...
  ans1_text_2 = new visual.TextStim({
    win: psychoJS.window,
    name: 'ans1_text_2',
    text: '',
    font: 'Arial',
  ...
  ans2_text_2 = new visual.TextStim({
    win: psychoJS.window,
    name: 'ans2_text_2',
    text: '',
    font: 'Arial',
  ...
  ans3_text_2 = new visual.TextStim({
    win: psychoJS.window,
    name: 'ans3_text_2',
    text: '',
    font: 'Arial',
  ...
  ans4_text_2 = new visual.TextStim({
    win: psychoJS.window,
    name: 'ans4_text_2',
    text: '',
    font: 'Arial',
  ...
  syn_ant_trialClock = new util.Clock();
...
function practice1_ansRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of practice1_ansComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function practice2_ansRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of practice2_ansComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function practice3RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of practice3Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function practice3_ansRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of practice3_ansComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function start_realRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of start_realComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
var gk_loop;
function gk_loopLoopBegin(gk_loopLoopScheduler, snapshot) {
  return async function() {
    TrialHandler.fromSnapshot(snapshot); // update internal variables (.thisN etc) of the loop
    
    // set up handler to look after randomisation of conditions etc
    gk_loop = new TrialHandler({
      psychoJS: psychoJS,
      nReps: 1, method: TrialHandler.Method.SEQUENTIAL,
      extraInfo: expInfo, originPath: undefined,
      trialList: 'gk_questions.xlsx',
      seed: undefined, name: 'gk_loop'
    });
    psychoJS.experiment.addLoop(gk_loop); // add the loop to the experiment
    currentLoop = gk_loop;  // we're now the current loop
    
    // Schedule all the trials in the trialList:
    for (const thisGk_loop of gk_loop) {
      snapshot = gk_loop.getSnapshot();
      gk_loopLoopScheduler.add(importConditions(snapshot));
      gk_loopLoopScheduler.add(gk_trialRoutineBegin(snapshot));
      gk_loopLoopScheduler.add(gk_trialRoutineEachFrame());
      gk_loopLoopScheduler.add(gk_trialRoutineEnd(snapshot));
      gk_loopLoopScheduler.add(gk_loopLoopEndIteration(gk_loopLoopScheduler, snapshot));
    }
...
async function gk_loopLoopEnd() {
  // terminate loop
  psychoJS.experiment.removeLoop(gk_loop);
  // update the current loop from the ExperimentHandler
  if (psychoJS.experiment._unfinishedLoops.length>0)
    currentLoop = psychoJS.experiment._unfinishedLoops.at(-1);
  else
...
function gk_loopLoopEndIteration(scheduler, snapshot) {
  // ------Prepare for next entry------
  return async function () {
    if (typeof snapshot !== 'undefined') {
      // ------Check if user ended loop early------
...
var syn_ant_loop;
function syn_ant_loopLoopBegin(syn_ant_loopLoopScheduler, snapshot) {
  return async function() {
    TrialHandler.fromSnapshot(snapshot); // update internal variables (.thisN etc) of the loop
    
    // set up handler to look after randomisation of conditions etc
    syn_ant_loop = new TrialHandler({
      psychoJS: psychoJS,
      nReps: 1, method: TrialHandler.Method.SEQUENTIAL,
      extraInfo: expInfo, originPath: undefined,
      trialList: 'syn_ant_questions.xlsx',
      seed: undefined, name: 'syn_ant_loop'
    });
    psychoJS.experiment.addLoop(syn_ant_loop); // add the loop to the experiment
    currentLoop = syn_ant_loop;  // we're now the current loop
    
    // Schedule all the trials in the trialList:
    for (const thisSyn_ant_loop of syn_ant_loop) {
      snapshot = syn_ant_loop.getSnapshot();
      syn_ant_loopLoopScheduler.add(importConditions(snapshot));
      syn_ant_loopLoopScheduler.add(syn_ant_trialRoutineBegin(snapshot));
      syn_ant_loopLoopScheduler.add(syn_ant_trialRoutineEachFrame());
      syn_ant_loopLoopScheduler.add(syn_ant_trialRoutineEnd(snapshot));
      syn_ant_loopLoopScheduler.add(syn_ant_loopLoopEndIteration(syn_ant_loopLoopScheduler, snapshot));
    }
...
async function syn_ant_loopLoopEnd() {
  // terminate loop
  psychoJS.experiment.removeLoop(syn_ant_loop);
  // update the current loop from the ExperimentHandler
  if (psychoJS.experiment._unfinishedLoops.length>0)
    currentLoop = psychoJS.experiment._unfinishedLoops.at(-1);
  else
...
function syn_ant_loopLoopEndIteration(scheduler, snapshot) {
  // ------Prepare for next entry------
  return async function () {
    if (typeof snapshot !== 'undefined') {
      // ------Check if user ended loop early------
...
var gc_introMaxDurationReached;
var _key_resp_allKeys;
var gc_introMaxDuration;
var gc_introComponents;
function gc_introRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'gc_intro' ---
    t = 0;
    gc_introClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    gc_introMaxDurationReached = false;
    // update component parameters for each repeat
    key_resp.keys = undefined;
    key_resp.rt = undefined;
    _key_resp_allKeys = [];
    psychoJS.experiment.addData('gc_intro.started', globalClock.getTime());
    gc_introMaxDuration = null
    // keep track of which components have finished
    gc_introComponents = [];
    gc_introComponents.push(text_10);
    gc_introComponents.push(key_resp);
    
    for (const thisComponent of gc_introComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function gc_introRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'gc_intro' ---
    // get current time
    t = gc_introClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
...
function gc_introRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('gc_intro.stopped', globalClock.getTime());
...
var gk_trialMaxDurationReached;
var gk_trialMaxDuration;
var gk_trialComponents;
function gk_trialRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'gk_trial' ---
    t = 0;
    gk_trialClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    gk_trialMaxDurationReached = false;
    // update component parameters for each repeat
    ...
    gotValidClick = false; // until a click is received
    psychoJS.experiment.addData('gk_trial.started', globalClock.getTime());
    gk_trialMaxDuration = null
    // keep track of which components have finished
    gk_trialComponents = [];
    gk_trialComponents.push(text_7);
    gk_trialComponents.push(ans1_text_2);
    gk_trialComponents.push(ans2_text_2);
    gk_trialComponents.push(ans3_text_2);
    gk_trialComponents.push(ans4_text_2);
    gk_trialComponents.push(mouse_11);
    
    for (const thisComponent of gk_trialComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function gk_trialRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'gk_trial' ---
    // get current time
    t = gk_trialClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    ...
    // *ans1_text_2* updates
    if (t >= 0.0 && ans1_text_2.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ans1_text_2.tStart = t;  // (not accounting for frame time here)
    ...
    // *ans2_text_2* updates
    if (t >= 0.0 && ans2_text_2.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ans2_text_2.tStart = t;  // (not accounting for frame time here)
    ...
    // *ans3_text_2* updates
    if (t >= 0.0 && ans3_text_2.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ans3_text_2.tStart = t;  // (not accounting for frame time here)
    ...
    // *ans4_text_2* updates
    if (t >= 0.0 && ans4_text_2.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ans4_text_2.tStart = t;  // (not accounting for frame time here)
    ...
    if (mouse_11.status === PsychoJS.Status.STARTED) {  // only update if started and not finished!
      ...
      if (!_mouseButtons.every( (e,i,) => (e == prevButtonState[i]) )) { // button state changed?
        ...
        if (_mouseButtons.reduce( (e, acc) => (e+acc) ) > 0) { // state changed to a new click
          // check if the mouse was inside our 'clickable' objects
          gotValidClick = false;
          mouse_11.clickableObjects = eval([ans1_text_2, ans2_text_2, ans3_text_2, ans4_text_2])
          ;// make sure the mouse's clickable objects are an array
...
function gk_trialRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('gk_trial.stopped', globalClock.getTime());
    // store data for psychoJS.experiment (ExperimentHandler)
...
var syn_ant_trialMaxDurationReached;
var syn_ant_trialMaxDuration;
var syn_ant_trialComponents;
function syn_ant_trialRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'syn_ant_trial' ---
    t = 0;
    syn_ant_trialClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    syn_ant_trialMaxDurationReached = false;
    // update component parameters for each repeat
    ...
    gotValidClick = false; // until a click is received
    psychoJS.experiment.addData('syn_ant_trial.started', globalClock.getTime());
    syn_ant_trialMaxDuration = null
    // keep track of which components have finished
    syn_ant_trialComponents = [];
    syn_ant_trialComponents.push(text_8);
    syn_ant_trialComponents.push(text_9);
    syn_ant_trialComponents.push(ans1_text);
    syn_ant_trialComponents.push(ans2_text);
    syn_ant_trialComponents.push(ans3_text);
    syn_ant_trialComponents.push(ans4_text);
    syn_ant_trialComponents.push(ans5_text);
    syn_ant_trialComponents.push(mouse_12);
    
    for (const thisComponent of syn_ant_trialComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function syn_ant_trialRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'syn_ant_trial' ---
    // get current time
    t = syn_ant_trialClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    ...
    // *ans4_text* updates
    if (t >= 0.0 && ans4_text.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ans4_text.tStart = t;  // (not accounting for frame time here)
    ...
    // *ans5_text* updates
    if (t >= 0.0 && ans5_text.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ans5_text.tStart = t;  // (not accounting for frame time here)
    ...
    if (mouse_12.status === PsychoJS.Status.STARTED) {  // only update if started and not finished!
      ...
      if (!_mouseButtons.every( (e,i,) => (e == prevButtonState[i]) )) { // button state changed?
        ...
        if (_mouseButtons.reduce( (e, acc) => (e+acc) ) > 0) { // state changed to a new click
          // check if the mouse was inside our 'clickable' objects
          gotValidClick = false;
          mouse_12.clickableObjects = eval([ans1_text, ans2_text, ans3_text, ans4_text, ans5_text])
          ;// make sure the mouse's clickable objects are an array
...
function syn_ant_trialRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('syn_ant_trial.stopped', globalClock.getTime());
    // store data for psychoJS.experiment (ExperimentHandler)
